...
def joint_mask_sentence(tokens, tokenizer,mlm_prob):
    ...
    ancient_ids = torch.tensor(ancient_ids)
    modern_ids = torch.tensor(modern_ids)
    masked_ancient = sentence_mask(tokenizer, ancient_ids, mlm_prob=mlm_prob)
    masked_modern = sentence_mask(tokenizer, modern_ids, mlm_prob=mlm_prob)
    inp_tensor = torch.cat([masked_ancient, masked_modern], dim=0)
    return inp_tensor
...
def get_unidirectional_tokenized_dataset(args, tokenizer, split='train'):
    ...
    def encode_fn(batch):
        ...
        # src_encode = tokenizer.batch_encode_plus(src_text,max_length = args.max_length,padding= 'max_length',return_tensors='pt',runcation=True).input_ids
        # tgt_encode = tokenizer.batch_encode_plus(tgt_text,max_length = args.max_length,padding= 'max_length',return_tensors='pt',truncation=True).input_ids
...
def get_AplusM_to_MplusA_tokenized_dataset(args,tokenizer,split='train'):
    ...
    def encode_fn(batch):
        ...
        src_text = modern_text+ancient_text
        tgt_text = ancient_text+modern_text
...
def get_MaMm2ma(args, tokenizer, split='train',mlm_prob=0.35):
    ...
    def encode_fn(batch):
        ...
        src_label =  [ancient_bos_id]+ src_label['input_ids'][1:-1] + [ancient_eos_id] 
...
# need change args.max_length=256 and model.max_position_embedding=512
def get_MaMm2partial(args, batch, tokenizer, mask= 'ancient',inp='modern',mlm_prob=0.35):
    def encode_fn(batch):
        src_text = batch[inp]
        tgt_text = batch[mask]
    ...
    fields = ['modern', 'ancient']
    dataset = batch.map(encode_fn).remove_columns(fields)
    return dataset
...
if __name__ == "__main__":
    ...
    encoded_testset = get_MaMm2partial(args, testset,tokenizer)
